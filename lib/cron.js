 ...
 *
 * Schedule:
 * 06:00 EAT — Morning scan: fetch fixtures + run v3.5 analysis
...
cron.schedule(
  ...
  async () => {
    console.log('\n' + '='.repeat(60));
    console.log(
      `[Cron] ☀️  MORNING SCAN — ${new Date().toLocaleString('sw-TZ', { timeZone: TZ })}`
    );
...
cron.schedule(
  ...
  async () => {
    console.log('\n' + '='.repeat(60));
    console.log(
      `[Cron] 🌙 EVENING RECONCILE — ${new Date().toLocaleString('sw-TZ', { timeZone: TZ })}`
    );
    console.log('='.repeat(60));
...
console.log('[Cron] ✅ MkekaBOT v3.5 scheduler active');
