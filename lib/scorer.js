...
function normalizeKickoff(kickoff) {
  ...
  if (/^\d{1,2}:\d{2}$/.test(kickoff)) return `${getAppDate()}T${kickoff}:00`;
  return kickoff;
}
...
export async function savePrediction(prediction, matchMeta) {
  ...
  const { homeTeam, awayTeam, kickoff, matchday, openingOdds, currentOdds, referee } = matchMeta;
  ...
  const sql = `
    INSERT INTO predictions (
      ...
      reasoning, audit_trail, warnings, verdict, should_bet, skip_reason,
      created_at
    ) VALUES (
      $1, $2, $3, $4, $5,
      $6, $7,
  ...
  const values = [
    getAppDate(),
    league ?? matchMeta.league ?? null,
    homeTeam?.name ?? match?.split(' vs ')[0],
    awayTeam?.name ?? match?.split(' vs ')[1],
    normalizeKickoff(kickoff),
    matchday ?? null,
    referee?.name ?? null,
    parseFloat(botLine) || null,
    Number.isFinite(parseFloat(davyLine)) ? parseFloat(davyLine) : null,
    Number.isFinite(parseFloat(confidence)) ? parseFloat(confidence) : null,
    Number.isFinite(parseFloat(marketOdds)) ? parseFloat(marketOdds) : null,
    Number.isFinite(parseFloat(openingOdds)) ? parseFloat(openingOdds) : null,
    Number.isFinite(parseFloat(currentOdds)) ? parseFloat(currentOdds) : null,
    reasoning ?? null,
    JSON.stringify(auditTrail ?? {}),
    JSON.stringify(warnings ?? []),
    verdict,
    verdict === 'BET',
    verdict === 'SKIP' ? prediction.reason ?? null : null,
  ];
...
export async function recordResult(predictionId, actualCards, homeCards, awayCards) {
  ...
  if (prediction.home_team)
    await updateTeamMomentumCache(prediction.home_team, prediction.league, homeCards);
  if (prediction.away_team)
    await updateTeamMomentumCache(prediction.away_team, prediction.league, awayCards);
...
export async function runEveningReconciliation() {
  ...
  for (const pred of pending.rows) {
    try {
      ...
      console.log(
        `[Reconcile] ${pred.home_team} vs ${pred.away_team}: ${result.totalCards} cards — ${outcome.correct ? 'WIN ✅' : 'LOSS ❌'} (line: ${pred.davy_line})`
      );
    } catch (err) {
  ...
  // Trigger Kimi self-learning if enough data
...
// ================================================================
// KIMI SELF-LEARNING (post-reconciliation analysis)
// Asks Kimi to identify patterns in wins/losses
// ================================================================
      ...
      messages: [
        {
          role: 'system',
          content:
            "You are MkekaBOT's self-learning module. Analyze prediction results to find patterns.",
        },
        {
          role: 'user',
...
async function fetchMatchResult(homeTeam, awayTeam, matchDate) {
  try {
    ...
    if (result?.success === false) {
      console.warn(`[fetchResult] Firecrawl reported failure for ${homeTeam} vs ${awayTeam}: ${result?.error}`);
      return null;
    }

    const statusCode = result?.metadata?.statusCode ?? result?.data?.metadata?.statusCode ?? null;
    if (statusCode && (statusCode < 200 || statusCode >= 300)) {
      console.warn(`[fetchResult] Source page returned HTTP ${statusCode} for ${homeTeam} vs ${awayTeam} - discarding`);
      return null;
    }

    const extract = result?.extract ?? result?.json ?? result?.data?.extract ?? result?.data?.json ?? null;
    if (extract && extract.matchFinished === false) return null;
    return extract;
  } catch (err) {
...
export async function getTodaysPredictions() {
  const result = await query(`
    ...
    WHERE match_date = $1::date
      AND kickoff >= $1::timestamp
      AND kickoff < ($1::date + INTERVAL '1 day')
    ORDER BY kickoff ASC
  `, [getAppDate()]);
  return result.rows;
}
...
export async function getStats(days = 30) {
  const result = await query(`
    ...
    FROM predictions
    WHERE match_date >= $1::date - ($2 * INTERVAL '1 day')
      AND league IS NOT NULL
    GROUP BY league
    ORDER BY league_win_rate DESC NULLS LAST
  `, [getAppDate(), parseInt(days)]);
  return result.rows;
}
...
export async function analyzeAndSaveFixtures(league = 'all') {
  let fixtures;
  let buildMatchDataFn;

  if (process.env.USE_CSV_DATA === 'true') {
    const { getCsvFixtures, buildMatchDataFromCsv } = await import('./csvData.js');
    fixtures = getCsvFixtures(league, 6);
    buildMatchDataFn = buildMatchDataFromCsv;
    console.log(`[Scorer] CSV mode: ${fixtures.length} fixtures`);
  } else {
    const { getTodaysFixtures, buildMatchData } = await import('./firecrawl.js');
    fixtures = await getTodaysFixtures(league);
    buildMatchDataFn = buildMatchData;
  }

  console.log(`[Scorer] Found ${fixtures.length} fixtures`);

  const { loadPredictor } = await import('./ai.js');
  const runPrediction = await loadPredictor();

  const results = [];

  for (const fixture of fixtures) {
    try {
      // Build full match data with v3.5 fields
      const matchData = await buildMatchDataFn(fixture, {
      ...
      // Run v3.5 algorithm
      const prediction = await runPrediction(matchData, marketOdds);
      ...
      console.error(
        `[Scorer] Error processing ${fixture.homeTeam} vs ${fixture.awayTeam}:`,
        err.message
      );
    }
  }
